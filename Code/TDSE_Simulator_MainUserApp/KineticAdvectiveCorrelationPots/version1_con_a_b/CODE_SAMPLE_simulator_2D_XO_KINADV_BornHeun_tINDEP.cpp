...
double xmin = -6.0, xmax = 14.0, ymin = -7.0, ymax = 7.0, t0=0.0, dt=0.001, posx, posy;
...
double eigenstatesForSectionsInx(double y, double x, int j){ //the so called psi_x^j(y)
    ...
    if(x<c || x>d){ L=ymax-ymin; o=ymin;} else{ L=2.0*s; o=-s; } 
...
double eigenstatesForSectionsIny(double x, double y, int j){ //the so called psi_y^j(x)
    double c=2.5, d=3.5, s=1.0, L, o;  
    if((x<c) & (y<-s || y>s)){L = c-xmin; o=xmin;}  else if((x>d) & (y<-s || y>s)){L = xmax-d; o=d;}  else{ L=xmax-xmin; o=xmin;}  
...
//We choose the a and b such that Uj(k) = aAj(k) + bBj(k) for k in {x,y}

double a_x=1.0, b_x=0.0, a_y=0.0, b_y=0.0;

// We declare/calculate the values if Bj
double Bx(double y, double x, int j){//y is y_alpha(t)
    double c=2.5, d=3.5, s=1.0, L; 
    if(x<c || x>d){ L=ymax-ymin;}else{ L=2.0*s; } 
    if(j%2==0){return 0.0;} else{return 2*sqrt(2*L)/(j*PI);}
}
double By(double x, double y, int j){//x is x_alpha(t)
    double c=2.5, d=3.5, s=1.0, L;  
    if((x<c) & (y<-s || y>s)){L = c-xmin;} else if((x>d) & (y<-s || y>s)){L = xmax-d;}  else{ L=xmax-xmin;} 
    if(j%2==0){return 0.0;} else{return 2*sqrt(2*L)/(j*PI);}
}
...
for(int it=0; it<timeIts; ++it){
    ...
    for(int i=0; i<=xDivs; ++i){
        ...
        for(int j=0; j<=xjmax; ++j){ //generate the kinetic and advective correlation potentials for this spatial grid point posx
            ...
            correlPot = correlPot + ( a_x*eigenstatesForSectionsInx(posy, posx, j) + b_x*Bx( posy, posx, j) )*(kineticCor + J*advectiveCor);
        }
    ...
    for(int i=0; i<=yDivs; ++i){
        ...
        for(int j=0; j<=yjmax; ++j){ //generate the kinetic and advective correlation potentials for this spatial grid point posx
            ...
            correlPot = correlPot + ( a_y*eigenstatesForSectionsIny(posx, posy, j) + b_y*By( posx, posy, j) )*(kineticCor + J*advectiveCor);
        }
